...
#define DEBUGPRINT2(x) do { if (debugflag>=2) { \
...
struct sparse_max_rank
{
    ...
    local_index_type index_to_rank(index_type j){
        ...
        mwIndex lind = index_to_lindex[j];
     ...
     */
    local_index_type rank_up(local_index_type j, int debugflag){
DEBUGPRINT2(("sparserank::rank_up : j_in = %i \n", j));       
        while (1) {
            if (j==0) { break; } 
...
DEBUGPRINT2(("sparserank::rank_up : val1 = %.8f , val2 = %.8f \n", values[listj2], values[listj]));            
                break;
...
DEBUGPRINT2(("sparserank::rank_up : j_out = %i \n", j));               
        return j;
    }
...
DEBUGPRINT2(("sparserank::update 1: index=%i , val=%0.8f , hsize=%i , nextind=%i \n", index, value, hsize, nextind));   
        if ( index_to_lindex.count(index) == 0 ) {
            ...
            nextind++ ;
DEBUGPRINT2(("sparserank::update 1.1: index=%i , lindex=%i , val=%0.8f , hsize=%i , nextind=%i \n", index, lindex, value, hsize, nextind));              
        } else {
        ...
        if ( L[lindex] == lastval ){
            ...
            L[lindex] = hsize;
            hsize++;
DEBUGPRINT2(("sparserank::update 2: index=%i , lindex=%i , val=%0.8f , hsize=%i , nextind=%i \n", index, lindex, value, hsize, nextind));  
            return rank_up(hsize-1, debugflag);
        }
        else{ // update old entry
            values[lindex] += value;
            start_index = L[lindex]; // entry already in: set start_index to rank j s.t. T[j] =  lindex
DEBUGPRINT2(("sparserank::update 2.1: index=%i , lindex=%i , start_index=%i , hsize=%i , nextind=%i \n", index, lindex, start_index, hsize, nextind));  
            return rank_up(start_index, debugflag);
        }
    }
};
