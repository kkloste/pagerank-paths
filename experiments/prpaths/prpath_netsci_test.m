...
%%
seed = 211; % chosen to illustrate certain points
...
% smallest_eps_gap = 30*epsmin ; % for faster testing
...
print(gcf,['./figures/netsci-path-rho', dummy ,'TEST.png'],'-dpng','-r600');
...
xlim(xl); % ensure conductance is on same axis as Path Plot above.
ylim([ min( rval.ep_stats(:,2)*0.9 ), 1] );
set(gca,'XTick',[10,100,1000,10000,100000]);
set_figure_size([3.5,1.5]);

print(gcf,['./figures/netsci-cond-rho', dummy ,'TEST.png'],'-dpng','-r600');
