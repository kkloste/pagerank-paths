 ...
 *
 * USAGE:
 * [bestset,cond,cut,vol] = ppr_fast_grid_mex(A,set,alpha,eps_min,theta,debugflag)
 *
 */
...
struct bestset_stats {
  ...
  void update_cluster_from_sweep(SolutionType p, std::vector<mwIndex>& p_nnzs) {  
    ...
    // to match the original ppr_grid_mex.cpp
...
struct ppr_grid {
    ...
    ppr_grid(const sparserow* _G, const double _alpha, const double _eps_min,
             const double _theta)
    : G(_G), alpha(_alpha), theta(_theta), eps_min(_eps_min)
    {}
...
void mexFunction(int num_outputs, mxArray* ptr_outputs[], int num_inputs, const mxArray* ptr_inputs[])
{
    ...
    DEBUGPRINT(
            ("ppr_grid_mex: inputs handled, call compute_pagerank "
             "(%i seeds, %.3f, %g, %g) \n", seeds.size(), alpha, eps_min, theta));

    //compute_pagerank(&G, seeds, alpha, eps_min, theta, stats, solnptr, soltracker);

    ppr_grid computer(&G, alpha, eps_min, theta);
