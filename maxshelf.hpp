...
class max_shelf {
    ...
    mwIndex max_size;
    std::vector< double > values;          // values[j] gives val of entry with index j
    std::vector< std::pair<mwIndex,mwIndex> > index2shelf; // index2shelf[j] gives pair for location in shelf of j    
    ...
    max_shelf(double _theta, double _eps_min, double _eps_max, mwIndex _max_size, int _debugflag)
    ...
    {
        values.resize(max_size,0.0);
        num_shelves = 1 + ceil(log2(_eps_min/_eps_max)/log2_theta);
    	lastval = std::numeric_limits<mwIndex>::max();
    	empty_pair = std::make_pair (lastval, lastval);
    	index2shelf.resize(max_size, empty_pair);
    }
    ...
    */
    ...
    mwIndex insert(mwIndex node_ID, double value){
        mwIndex shelf_ID = which_shelf(fabs(value));
        if (shelf_ID >= num_shelves){
            index2shelf[node_ID] = empty_pair; // set to outside the shelf
            return num_shelves;
        }       
    ...
    mwIndex update(mwIndex node_ID, double value){
        ...
        if ( index2shelf[node_ID] == empty_pair ){ return insert(node_ID, newval); }
