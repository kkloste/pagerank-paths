function [times conds seeds] = test_ppr_paths_rho(numtrials, filename, eps, alpha, rho, debugflag)
% [times conds seeds] = test_ppr_paths_rho_mex(numtrials, filename, eps, alpha, debugflag)
...
%
%   For mex only:
%     [step_stats,eps_stats,bestset] = ppr_ppr_paths_rho_mex(A,set,alpha,eps_min,debugflag);
...
assert(rho >= 0 && rho < 1, 'rho violates 1>rho>=0');
...
for j=1:length(seeds),
    tic;
    [step_stats,eps_stats,bestset] = ppr_paths_rho_mex(A,seeds(j),alpha, eps, rho, debugflag);
    times(j) = toc;
